                        ...
                        {data.currentUser.username == comment.user.username && (
                          <>
                            ...
                            <div
                              ...
                              onClick={() => toggleEditVisibility(comment.id)}
                            >
                                  ...
                                  <>
                                    <div
                                      style={{
                                        color: "#ed6368",
                                        position: "absolute",
                                        top: isMobile ? "85%" : "28px",
                                        right: isMobile ? "84px" : "105px",
...
function Edit({ commentId, storedData, setStoredData, commentContent }) {
  ...
  const editComment = () => {
    ...
    const areaComment = document.getElementById("commentText");
    const updatedData = { ...storedData };
    if (updatedData.comments.find((comment) => comment.id == commentId)) {
      updatedData.comments.find((comment) => comment.id == commentId).content =
        areaComment.value;
    } else {
      for (const comment of updatedData.comments) {
        const reply = comment.replies.find((reply) => reply.id === commentId);
        if (reply) {
          comment.replies.find((reply) => reply.id === commentId).content =
            areaComment.value;
          break;
        }
      }
          ...
          <div className="d-flex align-items-center flex-md-row">
            ...
            <div className="d-flex justify-content-between align-items-center flex-column">
              <button
                type="button"
                className="btn btn-primary"
                style={{ backgroundColor: "#5357b6", borderColor: "#5357b6" }}
                onClick={editComment}
              >
                Edit
              </button>
